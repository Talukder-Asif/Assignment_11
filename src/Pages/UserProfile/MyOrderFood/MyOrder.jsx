...
const MyOrder = () => {
  ...
  useEffect(() => {
    // axios.get(`https://testhalal-server.vercel.app/order/${user.email}`, {withCredentials: true} )
    axiosSecure.get(`/order/${user.email}`)
    .then((res) => {
  ...
  const handelDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // axios.delete(`https://testhalal-server.vercel.app/order/${id}`,{withCredentials: true} )
...
if (orderData.length === 0) {
  return <h1 className="text-4xl md:text-7xl font-bold">No Order Found</h1>;
}
  return (
